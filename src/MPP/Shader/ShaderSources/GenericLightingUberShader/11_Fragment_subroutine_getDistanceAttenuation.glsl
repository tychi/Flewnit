...
float 	getDistanceAttenuation(vec3 lightToFragW) 			
{
	...
	*/
	//return  50.0 / (sqrt( (lightToFragW.x * lightToFragW.x) + (lightToFragW.y * lightToFragW.y) + (lightToFragW.z * lightToFragW.z) ));
	return  1000.0 / (   ( (lightToFragW.x * lightToFragW.x) + (lightToFragW.y * lightToFragW.y) + (lightToFragW.z * lightToFragW.z) ));
}
